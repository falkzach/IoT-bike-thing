...
void writeAccel() {
	char g_forces[128];
	sprintf(g_forces, "%f,%f,%f\n", imu.calcAccel(imu.ax), imu.calcAccel(imu.ay), imu.calcAccel(imu.ax));
	appendFile(SD, "/data/gforce.txt", g_forces);
}
...
static void smartdelay(unsigned long ms) {
	...
	while (gps_serial.available() && (millis()-start<ms)) {
